...
description: Use this agent when working on CI/CD pipelines, containerization, infrastructure as code, build and release workflows, or developer tooling. This includes creating or modifying GitHub Actions workflows, Docker configurations, docker-compose files, Kubernetes manifests, Makefiles, Procfiles, or any automation that improves the development and deployment experience.\n\nExamples:\n\n<example>\nContext: User wants to set up automated testing in CI.\nuser: "We need to run our Go tests automatically on every pull request"\nassistant: "I'll use the devops-engineer agent to design and implement a GitHub Actions workflow for automated testing."\n<uses Task tool to launch devops-engineer agent>\n</example>\n\n<example>\nContext: User is having issues with the local development setup.\nuser: "The Procfile.dev isn't starting Redis properly"\nassistant: "Let me invoke the devops-engineer agent to diagnose and fix the local development configuration."\n<uses Task tool to launch devops-engineer agent>\n</example>\n\n<example>\nContext: User wants to containerize the application.\nuser: "Can we add Docker support for the backend?"\nassistant: "I'll use the devops-engineer agent to create Docker configurations that align with your existing setup."\n<uses Task tool to launch devops-engineer agent>\n</example>\n\n<example>\nContext: User needs to optimize build times.\nuser: "Our CI builds are taking too long, can we speed them up?"\nassistant: "I'll engage the devops-engineer agent to analyze the current pipeline and implement optimizations."\n<uses Task tool to launch devops-engineer agent>\n</example>
model: inherit
color: yellow
---
